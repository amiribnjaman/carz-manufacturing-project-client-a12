...
const ManageOrdersSingleRow = ({ index, order }) => {
    ...
    const handleDeleteItem = () => {
        const id = selectedId
        fetch(`https://carz-manufacturing-project-server-a12.vercel.app/order/${id}`, {
            method: 'DELETE'
        })
            .then(res => res.json())
            .then(data => {
            ...
            <td class="px-6 py-4 text-right">
                {
                    payment_status == false ? <>
                        ...
                        <p className='mr-2 font-medium text-green-500'>Unpaid</p>
                        <button
                            onClick={() => handleDeleteItemPopup(_id)}
                            className="font-medium hover:underline text-red-600 lg:pl-3 pl-1 inline-block">
                            Delete
                        </button>
                        {/* Confirmation popup */}
                        ...
                        : <div className=''>
                            <button
                                ...
                                class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{status}</button>
                        </div>
                }
            </td>
        </tr>
