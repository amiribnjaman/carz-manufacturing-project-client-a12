...
const MyOrdersSingleRow = ({ order, index }) => {
    const { _id, productName, price, quantity, payment_status, productId, transactionId } = order
            ...
            <td class="px-6 py-4 text-right">
                {
                    payment_status != false ? <>
                        <p class="font-medium text-green-500 ">paid</p>
                        <p>{transactionId}</p>
                    </>
                        : <>
                            <button
                                onClick={() => navigate(`/payment/${_id}`)}
                                class="font-medium mr-4 text-green-600 hover:underline"> Pay</button>

                            {/* Confirmation popup */}
                            <button
                                onClick={() => handleDeleteItemPopup(_id)}
                                className="font-medium mr-4 text-red-500 hover:underline">
                                Cancel Order
                            </button>
